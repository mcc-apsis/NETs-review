...
all_data_org <- all_data

data_wide <- all_data %>% 
  filter(measurement %in% c("min", "max", "estimate")) %>% 
  spread(key = measurement, value = value)

#estimate to "range"
data_wide$max[is.na(data_wide$max) & 
                !is.na(data_wide$estimate)] <-data_wide$estimate[is.na(data_wide$max) & !is.na(data_wide$estimate)] + 0.5
data_wide$min[is.na(data_wide$min) & 
                !is.na(data_wide$estimate)] <-data_wide$estimate[is.na(data_wide$min) & !is.na(data_wide$estimate)] - 0.5

data_wide$min[is.na(data_wide$min) & 
                !is.na(data_wide$max) & 
                  is.na(data_wide$estimate)] <- data_wide$max[is.na(data_wide$min)& 
                                                                !is.na(data_wide$max) & 
                                                                  is.na(data_wide$estimate)] - 1

data_wide$min[is.na(data_wide$min) & 
                !is.na(data_wide$max) & 
                  !is.na(data_wide$estimate) &
                data_wide$estimate < data_wide$max] <- data_wide$estimate[is.na(data_wide$min) & 
                                                                            !is.na(data_wide$max) &
                                                                              !is.na(data_wide$estimate)]

data_wide <- data_wide %>% 
  group_by(TI, variable, year, boundaries, year ) %>% 
  mutate(ind2 = row_number()) %>% 
  ungroup() %>% 
  mutate(
    hastotal = ifelse("Total" %in% `Data categorisationresource`, 1,0)
  ) %>% 
  gather(key = "measurement", value = "value", min, max, estimate, na.rm = T) %>% 
  group_by(TI,technology, variable, year) %>%
  arrange(value) %>%
  mutate(ind2 = row_number()) %>%
  filter(ind2 == 1 | ind2==n() | hastotal == 1) %>% 
  ungroup()

good_vars <- variable.names(all_data_org)
data_wide <- data_wide %>% 
  select(good_vars)
...
write.csv(all_sums, file = "../1.5_Sabine/interQ.csv")
...
all_sums$costs_max[all_sums$technology == "Enhanced weathering"] <-250
...
file = file.path(plotdir, paste0("panelA_shortX_250Lim.pdf"))
ggsave(file)
...
#==== SUBPLOTS ==========
plotdir <- paste0("plots/1.5_Sabine/Updated") #,u_sheetName) 
...
#}
save(data_bu, file = "../data_bu.Rdata")
